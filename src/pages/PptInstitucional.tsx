...
const defaultData = {
  ...
  methodology: [
    { step: "01", title: "Alinhamento Estratégico", desc: "Imersão completa no desafio: entendemos a cultura da empresa, o contexto da área, o perfil ideal e os critérios eliminatórios. Definimos juntos o escopo, cronograma e indicadores de sucesso.", duration: "Dia 1-2" },
    { step: "02", title: "Mapeamento & Hunting", desc: "Ativação da busca multicanal: LinkedIn Recruiter, base proprietária, IA preditiva e rede de contatos. Mapeamos o mercado de forma ativa, abordando inclusive profissionais que não estão em busca.", duration: "Dia 3-8" },
    { step: "03", title: "Triagem & Validação", desc: "Entrevistas por competência, avaliação técnica sob medida e análise de fit cultural. Cada candidato passa por um crivo rigoroso antes de chegar até você.", duration: "Dia 6-10" },
    { step: "04", title: "Shortlist & Parecer", desc: "Apresentação dos 3-5 candidatos finalistas com parecer individual detalhado: histórico profissional, motivações, expectativa salarial, pontos fortes e pontos de atenção.", duration: "Dia 10-12" },
    { step: "05", title: "Acompanhamento & Garantia", desc: "Suporte na negociação de proposta, acompanhamento do onboarding e monitoramento durante os primeiros 30 dias. Se houver desligamento no período de garantia, refazemos o processo sem custo.", duration: "Pós-contratação" },
  ],
  levels: [
...
const SlideFrame = ({ children, center }: { children: React.ReactNode; center?: boolean }) => (
  <div style={{ width: "100%", aspectRatio: "16/9", background: `linear-gradient(180deg,${B.bg},${B.bgAlt})`, position: "relative", overflow: "hidden", fontFamily: B.ff }}>
    <div style={{ position: "absolute", inset: 0, backgroundImage: `linear-gradient(${B.grid} 1px,transparent 1px),linear-gradient(90deg,${B.grid} 1px,transparent 1px)`, backgroundSize: "48px 48px" }} />
    <div style={{ position: "relative", zIndex: 1, height: "100%", display: "flex", flexDirection: "column", justifyContent: center ? "center" : "flex-start", alignItems: center ? "center" : "stretch", padding: "5% 6%" }}>
      {children}
    </div>
  </div>
);
...
const Crd = ({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) => (
  <div style={{ background: B.cardBg, border: `1px solid ${B.cardBd}`, borderRadius: 10, padding: "18px 20px", ...style }}>{children}</div>
);

const ST = ({ w, c }: { w: string; c: string }) => (
  <div style={{ display: "flex", alignItems: "baseline", gap: 5, marginBottom: 14 }}>
    <span style={{ color: B.cyan, fontSize: "1.5em", fontWeight: 800, letterSpacing: "-0.5px" }}>{w}</span>
    <span style={{ color: B.t2, fontSize: "0.85em" }}>{c}</span>
  </div>
);
...
function SlideCover({ data }: { data: SlideData }) {
  return (
    <SlideFrame center>
      <div style={{ fontSize: "1.1em", fontWeight: 800, letterSpacing: 3, color: B.cyan, marginBottom: 18, textTransform: "uppercase" }}>ORION Recruitment</div>
      <div style={{ fontSize: "0.55em", color: B.t2, marginBottom: 36, letterSpacing: 1 }}>Seu sucesso é o nosso sucesso.</div>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: "0.65em", color: B.t2, marginBottom: 6 }}>Proposta Comercial Exclusiva para:</div>
        <div style={{ fontSize: "1.8em", fontWeight: 800, color: B.white, lineHeight: 1.15 }}>{data.clientName || "Sua Empresa"}</div>
        {data.contactName && <div style={{ fontSize: "0.7em", color: B.t1, marginTop: 10 }}>A/C {data.contactName}{data.contactRole ? ` — ${data.contactRole}` : ""}</div>}
      </div>
    </SlideFrame>
  );
}
...
function SlideAbout({ data }: { data: SlideData }) {
  return (
    <SlideFrame>
      <Crd>
        <ST w="Quem" c="Somos" />
        <div style={{ color: B.t1, fontSize: "0.72em", lineHeight: 1.7, marginBottom: 20 }}>{data.aboutText}</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
          {data.aboutNumbers.map((n, i) => (
            <div key={i} style={{ textAlign: "center", background: B.cDim, borderRadius: 8, padding: "12px 6px" }}>
              <div style={{ color: B.cyan, fontSize: "1.3em", fontWeight: 800 }}>{n.value}</div>
              <div style={{ color: B.t2, fontSize: "0.55em", marginTop: 3 }}>{n.label}</div>
            </div>
          ))}
        </div>
      </Crd>
      ...
      <Crd>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          {data.differentials.map((d, i) => (
            <div key={i} style={{ background: B.cGlow, border: `1px solid ${B.cBord}`, borderRadius: 8, padding: "14px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: "1.2em" }}>{d.icon}</span>
                <div>
                  <div style={{ color: B.white, fontWeight: 700, fontSize: "0.72em" }}>{d.title}</div>
                  <div style={{ color: B.t2, fontSize: "0.55em", lineHeight: 1.5, marginTop: 3 }}>{d.desc}</div>
                </div>
              </div>
            </div>
          ))}
          ...
          {data.services.map((s, i) => (
            <div key={i} style={{ background: B.cGlow, border: `1px solid ${B.cBord}`, borderRadius: 8, padding: "14px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 6 }}>
                ...
                <span style={{ color: B.white, fontWeight: 700, fontSize: "0.72em" }}>{s.title}</span>
              </div>
              <div style={{ color: B.t1, fontSize: "0.55em", lineHeight: 1.55, marginBottom: 5 }}>{s.desc}</div>
              <div style={{ color: B.cyan, fontSize: "0.5em", fontStyle: "italic" }}>{s.details}</div>
            </div>
          ))}
        </div>
      </Crd>
  ...
  return (
    <SlideFrame>
      <Crd>
        ...
        <div style={{ color: B.t2, fontSize: "0.55em", marginBottom: 14 }}>Nossas verticais de atuação — Parte {half}/2</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
          {items.map((s, i) => (
            <div key={i} style={{ background: B.cGlow, border: `1px solid ${B.cBord}`, borderRadius: 8, padding: "14px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                <span style={{ fontSize: "1.1em" }}>{s.icon}</span>
                <span style={{ color: B.white, fontWeight: 700, fontSize: "0.7em" }}>{s.area}</span>
              </div>
              <div>
                {s.items.map((it, j) => (
                  <div key={j} style={{ color: B.t1, fontSize: "0.5em", lineHeight: 1.8, display: "flex", gap: 5 }}>
      ...
      <Crd>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {data.methodology.map((m, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, background: B.cGlow, border: `1px solid ${B.cBord}`, borderRadius: 8, padding: "12px 16px" }}>
              <span style={{ color: B.cyan, fontWeight: 800, fontSize: "1em", minWidth: 28 }}>{m.step}</span>
              <div style={{ flex: 1 }}>
                <div style={{ color: B.white, fontWeight: 700, fontSize: "0.7em" }}>{m.title}</div>
                <div style={{ color: B.t2, fontSize: "0.5em", lineHeight: 1.55, marginTop: 3 }}>{m.desc}</div>
              </div>
              <span style={{ color: B.cyan, fontSize: "0.5em", fontWeight: 600, whiteSpace: "nowrap" }}>{m.duration}</span>
            </div>
          ))}
        </div>
      </Crd>
  ...
  return (
    <SlideFrame>
      <Crd>
        ...
        <div style={{ color: B.t2, fontSize: "0.55em", marginBottom: 14 }}>Da base operacional até a gerência — cada nível recebe uma abordagem de hunting adequada à complexidade.</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {data.levels.map((l, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, background: B.cGlow, border: `1px solid ${B.cBord}`, borderRadius: 8, padding: "12px 16px" }}>
              <div style={{ flex: 1 }}>
                <div style={{ color: B.white, fontWeight: 700, fontSize: "0.7em" }}>{l.level}</div>
                <div style={{ color: B.t2, fontSize: "0.5em" }}>{l.examples}</div>
              </div>
              <div style={{ textAlign: "center", minWidth: 70 }}>
                <div style={{ color: B.cyan, fontWeight: 800, fontSize: "0.8em" }}>{l.fee}</div>
                <div style={{ color: B.t3, fontSize: "0.45em" }}>Honorário</div>
              </div>
              <div style={{ textAlign: "center", minWidth: 70 }}>
                <div style={{ color: B.cyan, fontWeight: 800, fontSize: "0.8em" }}>{l.sla}</div>
                <div style={{ color: B.t3, fontSize: "0.45em" }}>SLA</div>
              </div>
            </div>
          ))}
        </div>
    ...
    <SlideFrame>
      <Crd>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr", gap: 20 }}>
          <div style={{ background: B.cGlow, border: `1px solid ${B.cBord}`, borderRadius: 10, padding: "24px 20px", textAlign: "center" }}>
            <div style={{ color: B.cyan, fontSize: "2em", fontWeight: 800 }}>{data.investmentFee}</div>
            <div style={{ color: B.t1, fontSize: "0.65em", marginTop: 4 }}>Honorário no Sucesso</div>
          </div>
          <div>
            <div style={{ color: B.white, fontWeight: 700, fontSize: "0.75em", marginBottom: 8 }}>Sem Custos Antecipados</div>
            <div style={{ color: B.t1, fontSize: "0.55em", lineHeight: 1.65, marginBottom: 10 }}>{data.investmentText}</div>
            <div style={{ color: B.t2, fontSize: "0.52em", lineHeight: 1.6, marginBottom: 10 }}>
              O valor do honorário é equivalente a {data.investmentFee} {data.investmentDesc} acordada com o profissional escolhido.
            </div>
            <div style={{ background: B.cDim, borderRadius: 6, padding: "8px 12px" }}>
              <div style={{ color: B.t2, fontSize: "0.48em", lineHeight: 1.6 }}>
  ...
  return (
    <SlideFrame>
      <Crd>
        ...
        <div style={{ color: B.t2, fontSize: "0.55em", marginBottom: 14 }}>Operamos em todo o Brasil com processos híbridos (remoto + presencial), mantendo o mesmo padrão de qualidade e velocidade.</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10, marginBottom: 16 }}>
          {data.coverageRegions.map((r, i) => (
            <div key={i} style={{ textAlign: "center", background: r.highlight ? B.cDim : "transparent", border: `1px solid ${r.highlight ? B.cyan : B.cBord}`, borderRadius: 8, padding: "12px 8px" }}>
              <div style={{ color: r.highlight ? B.cyan : B.t1, fontWeight: 700, fontSize: "0.7em" }}>{r.region}</div>
              <div style={{ color: B.t2, fontSize: "0.48em", marginTop: 3 }}>{r.states}</div>
            </div>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
          {[{ v: "26", l: "Estados atendidos + DF" }, { v: "100%", l: "Processos remote-friendly" }, { v: "48h", l: "Kickoff após alinhamento" }].map((s, i) => (
            <div key={i} style={{ textAlign: "center" }}>
              <div style={{ color: B.cyan, fontSize: "1.2em", fontWeight: 800 }}>{s.v}</div>
              <div style={{ color: B.t2, fontSize: "0.5em" }}>{s.l}</div>
            </div>
          ))}
        </div>
      </Crd>
...
function SlideCTA({ data }: { data: SlideData }) {
  return (
    <SlideFrame center>
      <div style={{ fontSize: "1.6em", fontWeight: 800, color: B.white, marginBottom: 14, textAlign: "center", lineHeight: 1.15 }}>Vamos transformar seu time?</div>
      <div style={{ color: B.t1, fontSize: "0.65em", textAlign: "center", maxWidth: "70%", lineHeight: 1.65, marginBottom: 24 }}>{data.ctaSubtitle}</div>
      <div style={{ color: B.cyan, fontSize: "0.7em", fontWeight: 700 }}>{data.website}</div>
      {(data.email || data.phone) && (
        <div style={{ display: "flex", gap: 20, marginTop: 10 }}>
          {data.email && <span style={{ color: B.t2, fontSize: "0.55em" }}>{data.email}</span>}
          {data.phone && <span style={{ color: B.t2, fontSize: "0.55em" }}>{data.phone}</span>}
        </div>
      )}
    </SlideFrame>
  );
...
export default function PptInstitucional() {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        const dataUrl = await toPng(slides[i], { quality: 1, pixelRatio: 2, width: slides[i].offsetWidth, height: slides[i].offsetHeight });
        const img = new Image();
  ...
  if (mode === "preview") return (
    <div className="min-h-screen" style={{ background: "#070e18", fontFamily: B.ff }}>
      ...
      <div ref={previewRef} style={{ maxWidth: 940, margin: "0 auto", padding: "20px 0 70px" }}>
        {vis.map(s => (
