...
BEGIN
    ...
    v_linkedin_norm := CASE
        WHEN p_linkedin_url IS NOT NULL AND p_linkedin_url != '' THEN LOWER(TRIM(REGEXP_REPLACE(p_linkedin_url, '/+$', '')))
        ELSE NULL
    END;
    ...
    -- 3. Busca por LinkedIn URL normalizada
    IF v_linkedin_norm IS NOT NULL THEN
        SELECT id INTO v_party_id
        FROM public.party
        WHERE LOWER(TRIM(REGEXP_REPLACE(linkedin_url, '/+$', ''))) = v_linkedin_norm AND status = 'active'
        LIMIT 1;
